...
if  [ ! -n "$UMI_ENV" ] ;then
    output="test-umi-zip-dist"
else
    output="${UMI_ENV}-test-umi-zip-dist"
fi

# 删除之前打包好的项目

rm -rf ./*-zip-dist ./dist ./test-umi*.zip
mkdir ./${output}
mkdir ./${output}/bin
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/test-umi/'${UMI_ENV}'_test-umi/g' ./${output}/package.json
  else 
    sed -i 's/test-umi/'${UMI_ENV}'_test-umi/g' ./${output}/package.json
  fi
fi
...
if test -n "$UMI_ENV"; then
  if [ "$(uname)" == "Darwin" ];then
    sed -i '' 's/test-umi/'${UMI_ENV}'_test-umi/g' ./${output}/bin/stop.sh
  else 
    sed -i 's/test-umi/'${UMI_ENV}'_test-umi/g' ./${output}/bin/stop.sh
  fi
fi
